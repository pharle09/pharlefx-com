...
function open(socket: WebSocket): Promise<void> {
    return new Promise((resolve, reject) => { const timer = setTimeout(() => reject(new Error('Deriv connection timed out.')), 15000); socket.addEventListener('open', () => { clearTimeout(timer); resolve(); }, { once: true }); socket.addEventListener('error', () => { clearTimeout(timer); reject(new Error('Deriv connection failed.')); }, { once: true }); });
}

function send(socket: WebSocket, payload: object): Promise<DerivMessage> {
    return new Promise((resolve, reject) => { const timer = setTimeout(() => { socket.removeEventListener('message', onMessage); reject(new Error('Deriv request timed out.')); }, 15000); const onMessage = (event: MessageEvent) => { clearTimeout(timer); socket.removeEventListener('message', onMessage); const response = JSON.parse(String(event.data)) as DerivMessage; if (response.error) reject(new Error(response.error.message || 'Deriv request failed.')); else resolve(response); }; socket.addEventListener('message', onMessage); socket.send(JSON.stringify(payload)); });
}
